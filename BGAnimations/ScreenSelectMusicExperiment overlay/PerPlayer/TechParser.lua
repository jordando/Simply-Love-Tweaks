...
local function ParseTech(lines)
    ...
    Trace("Begin parsing chart")
    Trace("Peak to check against is:")
    ...
    while i < #chart+1 do
        ...
        if (chart[i]:match("^[,;]%s*")) then
            measure = measure + 1
            Trace("New Measure: "..measure)
            Trace("Beat is: "..beat.new)
