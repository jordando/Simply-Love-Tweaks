...
local bg = Def.Quad{
	InitCommand=function(self)
		self:zoomto(width, height)
			:align(0,0)
			:diffusetopedge(color("#23279e")):diffusebottomedge(Color.Black)
		if (NoteFieldIsCentered and IsUsingWideScreen()) then
			self:addx(-8):zoomto(width+14, height)
		end
	end
}

local frame = Def.Sprite{
	Texture=THEME:GetPathG("FF","CardEdge.png"),
	InitCommand=function(self)
		self:zoomto(width+46,height+10):align(0,0)
		self:xy(-22,-5)
		if (NoteFieldIsCentered and IsUsingWideScreen()) then
			self:zoomtowidth(width+44,height+10)
		end
	end,
}
