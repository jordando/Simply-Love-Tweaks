...
local player, side, ComputedData = unpack(...)
...
if fapping then
	...
	table.insert(abbreviations[SL.Global.GameMode],2,"Fan")
...
if fapping then num_judgments_available = num_judgments_available + 1 end
-- ---------------------------------------------
...
for i=2,num_judgments_available do
	local incrementer = fapping and i - 1 or i
...
if next(offsets) ~= nil then
	...
	if ComputedData and ComputedData.Histogram then
		...
		histogram = LoadActor("./Calculations.lua", {player, offsets, worst_window, pane_width, pane_height, colors})
