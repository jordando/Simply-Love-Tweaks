...
local footBreakdown = args.footBreakdown
...
for i, column in ipairs( cols ) do
	...
	for j, judgment in ipairs(rows) do
		...
		if gmods.TimingWindows[j] or j==#rows then
			...
			af[#af+1] = LoadFont("Common Normal")..{
				InitCommand=function(self)
					self:xy(_x, j*row_height + 4)
						:zoom(0.9)
					if side then
						if footBreakdown[side][column.Track][j] then
							self:settext(footBreakdown[side][column.Track][j].count)
						elseif j == #rows and footBreakdown[side][column.Track]["Miss"] then
							self:settext(footBreakdown[side][column.Track]["Miss"].count)
						else self:settext("0") end
					else
			...
			if side then
				af[#af+1] = LoadFont("Common Normal")..{
					--Text=footBreakdown[side][column.Track][j].count,
					InitCommand=function(self)
						self:xy(_x, j*row_height):zoom(0.65):halign(0)
						if footBreakdown[side][column.Track][j] then
							local earlies = footBreakdown[side][column.Track][j].early
							local total = footBreakdown[side][column.Track][j].count
							local percent = earlies/total * 100
					...
					OnCommand=function(self)
						if j ~= #rows then
							self:x( self:GetX() + bmt[j]:GetWidth()/2 )
						end
					end
				}
			end
